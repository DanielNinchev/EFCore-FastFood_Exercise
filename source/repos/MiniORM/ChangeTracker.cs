...
namespace MiniORM
{
	internal class ChangeTracker<T> where T: class, new()
    {
        ...
        //it should not do any changes in the database
        ...
        public void Remove(T item) => this.removed.Add(item); //marks the item for removal, but it doesn't remove it yet from the database;
        ...
        public IEnumerable<T> GetModifiedEntities(DbSet<T> dbSet)
        {
            ...
            PropertyInfo[] primaryKeys = typeof(T)
                .GetProperties()
                .Where(pi => pi.HasAttribute<AssemblySignatureKeyAttribute>())
                .ToArray();
